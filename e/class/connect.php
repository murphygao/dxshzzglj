...
{
    global $ecms_config;
    $dblink = do_dbconnect($ecms_config['db']['dbserver'], $ecms_config['db']['dbport'],
        $ecms_config['db']['dbusername'], $ecms_config['db']['dbpassword'], $ecms_config['db']['dbname']);
    return $dblink;
}
...
function eReturnRewriteInfoUrl($classid, $id, $ecms = 0)
{
    ...
    if (empty($public_r['rewriteinfo'])) {
        ...
        if ($ecms == 1) {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array('[!--classid--]', '[!--id--]', '[!--page--]'),
                    array($classid, $id, 0), $public_r['rewriteinfo']);
        } else {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array('[!--classid--]', '[!--id--]'),
                    array($classid, $id), $public_r['rewriteinfo']);
        }
        $r['rewrite'] = 1;
    }
    return $r;
...
function eReturnRewriteClassUrl($classid, $ecms = 0)
{
    ...
    if (empty($public_r['rewriteclass'])) {
        ...
        if ($ecms == 1) {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array('[!--classid--]', '[!--page--]'),
                    array($classid, 0), $public_r['rewriteclass']);
        } else {
...
function eReturnRewriteTitleTypeUrl($ttid, $ecms = 0)
{
    ...
    if (empty($public_r['rewriteinfotype'])) {
        ...
        if ($ecms == 1) {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array('[!--ttid--]', '[!--page--]'), array($ttid, 0),
                    $public_r['rewriteinfotype']);
        } else {
...
function eReturnRewriteTagsUrl($tagid, $tagname, $ecms = 0)
{
    ...
    if (empty($public_r['rewritetags'])) {
        ...
        if ($ecms == 1) {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array('[!--tagname--]', '[!--page--]'),
                    array($tagname, 0), $public_r['rewritetags']);
        } else {
...
function eReturnRewritePlUrl($classid, $id, $doaction = 'doinfo', $myorder = 0, $tempid = 0, $ecms = 0)
{
    ...
    if (empty($public_r['rewritepl'])) {
        ...
        if ($ecms == 1) {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array(
                    '[!--doaction--]',
                    '[!--classid--]',
                    '[!--id--]',
                    '[!--page--]',
                    '[!--myorder--]',
                    '[!--tempid--]'
                ), array($doaction, $classid, $id, 0, $myorder, $tempid), $public_r['rewritepl']);
        } else {
            $r['pageurl'] = $public_r['newsurl'] . str_replace(array(
                    '[!--doaction--]',
                    '[!--classid--]',
                    ...
                    '[!--myorder--]',
                    '[!--tempid--]'
                ), array($doaction, $classid, $id, $myorder, $tempid), $public_r['rewritepl']);
        }
        $r['rewrite'] = 1;
    }
    return $r;
...
function InfoUsePage($num, $line, $page_line, $start, $page, $search, $add)
{
    ...
    if ($page != $totalpage - 1) {
        ...
        $lastpage = '&nbsp;<a href="' . eReturnRewritePageLink($add,
                $totalpage - 1) . '">' . $fun_r['lastpage'] . '</a>';
    }
    $starti = $page - $snum < 0 ? 0 : $page - $snum;
    $no = 0;
    for ($i = $starti; $i < $totalpage && $no < $page_line; $i++) {
...
function ReturnClassLink($classid)
{
    ...
    for ($i = 1; $i < count($r) - 1; $i++) {
        ...
        if ($i == (count($r) - 2)) {
            $string .= "<li class=\"active\">" . $class_r[$r[$i]][classname] . "</li>";
        } else {
            $string .= "<li><a href=\"" . $url . "\">" . $class_r[$r[$i]][classname] . "</a></li>";
        }
...
function DoUpdateAddDataNum($type = 'info', $stb, $addnum = 1)
{
    ...
    if ($stb) {
        ...
        if (strstr($pur[$lastnumtbf], '|' . $stb . ',')) {
            ...
            $tbnums = str_replace('|' . $stb . ',' . $numrt[0] . '|', '|' . $stb . ',' . $newnum . '|',
                $pur[$lastnumtbf]);
        } else {
...
function ReturnSqlListF($mid)
{
    ...
    $f = 'id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard' . substr($emod_r[$mid]['listtempf'],
            0, -1);
    return $f;
}
...
function ReturnSqlTextF($mid, $ecms = 0)
{
    ...
    $f = ($ecms == 0 ? 'id,classid,' : '') . 'ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard' . substr($emod_r[$mid]['tbmainf'],
            0, -1);
    return $f;
}
...
function SubSmalltextVal($value, $len)
{
    ...
    $value = str_replace(array("\r\n", "<br />", "<br>", "&nbsp;", "[!--empirenews.page--]", "[/!--empirenews.page--]"),
        array("", "\r\n", "\r\n", " ", "", ""), $value);
...
function DoTranUrl($url, $classid)
{
    ...
    if (strstr($ecms_config['sets']['tranflashtype'], ',' . $r[filetype] . ',')) {
        ...
        $r[type] = 1;
    } elseif (strstr($ecms_config['sets']['mediaplayertype'],
            ',' . $r[filetype] . ',') || strstr($ecms_config['sets']['realplayertype'], ',' . $r[filetype] . ',')
    )//多媒体
    {
        $r[type] = 3;
    } else {
